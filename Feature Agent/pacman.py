...
def readCommand(argv):
    ...
    parser.add_option('-s', '--savename', dest='savename', help=default('the name under which this run will be stored. Defaults to None, in which case no save is made. When used with a test agent, determines the model that will be loaded.'), default=None)
    parser.add_option('-v', '--csv', dest='csvfile', help=default('location of a csvfile to which results of a test will be appended'), default=None)
    ...
    agentOpts['savename'] = options.savename
    
    global csvfile, savename
    csvfile = None
    if options.csvfile:
    ...
    pacman.savename = options.savename
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    if (numGames - numTraining) > 0:
        ...
        if csvfile:
            outfile = csv.writer(open(csvfile,'a',newline='\n'),delimiter=',')
            outfile.writerow([savename,
                              min(scores),
