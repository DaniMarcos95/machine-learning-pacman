...
class DeepQNetwork:
    ...
    def __init__(self):
        ...
        self.x = tf.placeholder('float', [None, 11, 20, 3])
        ...
        def conv_net(x):  
            ...
            conv3_flat = tf.reshape(conv3, [-1, 11 * 20 * 256])
        ...
        self.optim = tf.train.AdamOptimizer(0.00025).minimize(self.cost)
    ...
    def train(self,bat_s,bat_a,bat_t,bat_n,bat_r):
            ...
            _,cost = self.sess.run([self.optim, self.cost],feed_dict=feed_dict)
            return cost
